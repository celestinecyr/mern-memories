...
const Form = ( {currentId, setCurrentId} ) => {
    const [postData, setPostData] = useState({creator:'', title:'', message:'', tags:'', selectedFile:''})  // properties that the object is going to start with
    ...
    const handleSubmit = (e) => {
        ...
        if(currentId) {                                 //if id is not null, we wont create another post, we will update
            dispatch(updatePost(currentId, postData));
        } else{
            dispatch(createPost(postData));             //dispatch an action
        }
        clear();                                        //clearing inputs also means changing currentId --> as soon as we change the current id, in the app is gg to dispatch the getpost action (App.js line 18)
    } 

    const clear = () => {
        setCurrentId(null);                                                             //setting currentId to null - reset
        setPostData({creator:'', title:'', message:'', tags:'', selectedFile:''});      //save everything as empty string
    }
